...
test_size = np.linspace(0.1, 0.9, 143)
nn = np.arange(1, 28600, 200)
#
...
for i in range(samples):
    for s in range(len(test_size)):
        for n in range(len(nn)):
            if nn[n] + 1 > 28632 - test_size[s]*28632:
                break
            phase_diagram[s][n] += test_accuracy(x, y, nn[n], test_size[s])
